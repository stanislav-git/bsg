...
//		document.getElementById('new_fl').style.display='none';
		...
		echo "<form method='post'><p>Вы покидаете флот ",$cfl_name,"!<br>Укажите название Вашего флота: <input type='text' name='newname_fl' value='",$ship_data['s_name'],"'><br><small>(нельзя использовать названия других кораблей, и названия существующих флотов)</small>";
...
from ships join typeship on ships.type=typeship.id where ships.id<>? and ships.fleet=? and ships.repair=0 order by typeship.sizz,typeship.purp LIMIT 10");
	        ...
	        echo "<hr></p>";
		echo "<input type='submit' name='new_fl_' value='ПРОИЗВЕСТИ АРЕСТ'></form>";
	}
	if ($_POST['act']=='ch_fl'){
...
WHERE ships.id=? AND t1.who<>t2.who");
		...
		if ($q_fl->rowcount()>0){
			echo "<form method='post'><p>Произвести переход во флот: <select name='new_fl'><option disabled selected>Выбор флота</option>";
			...
			echo "<input type='submit' name='ch_fl_' value='ПРОИЗВЕСТИ АРЕСТ'></form>";
		} else {
...
WHERE (project.ship IS NULL OR project.timer<UNIX_TIMESTAMP(NOW())) AND typeship.purp=5 AND ships.repair=0 AND ships.fleet=?");
		...
		echo "<p>Произвести демонтаж корабля ",$ship_data['s_name']," и расселение ",$ship_data['human']," человек по кораблям флота?<hr></p>";
	        echo "<form method='post'><input type='hidden' name='fleet' value='",$_POST['fl'],"'><input type='hidden' name='ids' value='",$_POST['idship'],"'>";
	...
	if ($_POST['act']=='peres'){
		if ($ship_data['purp']==3){
		echo "<form method='post'><p>Инициировать проект перестройки корабля ",$ship_data['s_name']," в ";
		echo "<select name='purp'><option disabled selected>Новый собственник</option>";
		$q_type=$pdo->prepare("select type,purp from typeship where sizz=? and purp<>?");
		$q_type->execute(array($ship_data['sizz'],$ship_data['purp']));
		while ($type=$q_type->fetch()){
			echo "<option value='",$type['purp'],"'>",$type['type'],"</option>";
		}
		echo "</select>";
	        echo "<input type='hidden' name='fleet' value='",$_POST['fl'],"'><input type='hidden' name='ids' value='",$_POST['idship'],"'>";
	...
	if ($_POST['act']=='ch_vlad'){
		echo "<form method='post'><p>Настоящим, Вы соглашаетесь передать корабль ",$ship_data['s_name']," c ",$ship_data['human']," людьми на борту,<br> в собственность: ";
		$q_vlad=$pdo->prepare('select id,name from users where id_f=?');
		$q_vlad->execute([$_POST['fl']]);
...
round(ships.human*0.2*norms.n2/100) as h_water, round(ships.human*0.1*norms.n3/100) as h_comp, ships.descparts as descp, ships.spec as spec,
dig.`resurs` AS res,ships.`repair` AS rep, ships.user as id_u, ships.fleet as f_id
from ships
join typeship on ships.type=typeship.id 
